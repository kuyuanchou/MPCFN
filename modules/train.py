            ...
            with tqdm(total=len(self.dev_data), leave=False, dynamic_ncols=True) as pbar:
                ...
                if micro_f1 >= self.best_dev_metric:  # this epoch get best performance
                    ...
                    if self.args.save_path is not None:
                        torch.save(self.model.state_dict(), self.args.save_path + "/17_best_model.pth")
...
class NERTrainer(BaseTrainer):
    ...
    def test(self):
        ...
        if self.args.load_path is not None:  # load model from load_path
            self.logger.info("Loading model from {}".format(self.args.load_path))
            self.model.load_state_dict(torch.load(self.args.load_path, map_location="cuda"))  #
            self.logger.info("Load model successful!")
        sentneces, y_true, y_pred = [], [], []
        with torch.no_grad():
            with tqdm(total=len(self.test_data), leave=False, dynamic_ncols=True) as pbar:
                pbar.set_description_str(desc="Testing")
                total_loss = 0
                for batch in self.test_data:
                    ...
                    input_ids, attention_mask, labels, logits, loss = self._step(batch,
                                                                                 mode="test")  # logits: batch, seq, num_labels
                    total_loss += loss.detach().cpu().item()
                    ...
                    for row, mask_line in enumerate(input_mask):
                        ...
                        valid_input_ids = [input_ids[row][i] for i, m in enumerate(input_mask[row]) if m]
                        sentence = self.tokenizer.decode(valid_input_ids, skip_special_tokens=True)
                ...
                # pre_result = ""
                # for sentence, pre, label in zip(sentneces, y_pred, y_true):
                #     pre_result += "".join(sentence) + "\n"
                #     pre_result += " ".join(pre) + "\n"
                #     pre_result += " ".join(label) + "\n" + "\n"
                # with open(self.args.save_pre_result_path, mode="w", encoding="UTF-8") as f:
                #     f.write(pre_result)
                ...
                with open(self.args.save_result_path, mode="a", encoding="UTF-8") as f:
                    f.write(results)
        self.model.train()
    ...
    def _step(self, batch, mode="train"):
        ...
        if mode == "test":
            return input_ids, attention_mask, labels, logits, loss
    ...
    def multiModal_before_train(self):
        ...
        # prompt lr
        params = {'lr': self.args.lr, 'weight_decay': 5e-2}  # 5e-2
        params['params'] = []
        for name, param in self.model.named_parameters():
            if 'encoder_conv' in name or 'gates' in name:
                params['params'].append(param)
